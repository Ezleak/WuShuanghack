...
public class Instant extends Module {
    ...
    private static void rend(){
        ...
        public void onRenderWorld(RenderEvent event) {
            ...
            if (this.render.getValue() && this.breakPos != null) {
                Color color = new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue());
                RenderUtil.boxESP(this.breakPos, color, this.lineWidth.getValue(), this.outline.getValue(), this.box.getValue(), this.boxAlpha.getValue(), true);
            }
        }
    ...
    public void update()
    {
        if (nullCheck())
            return;
        if (breakPos != null && (this.ghostHand.getValue())) {
            if (this.ghostHand.getValue() && mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() != Items.DIAMOND_PICKAXE)
                return;
            mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, breakPos, facing));
            timer.reset();
        }
