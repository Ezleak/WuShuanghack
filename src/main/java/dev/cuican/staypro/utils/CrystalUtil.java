...
public class CrystalUtil {
    ...
    public static boolean canPlaceCrystal(final BlockPos blockPos, final boolean newPlace) {
        final BlockPos boost = blockPos.add(0, 1, 0);
        final BlockPos boost2 = blockPos.add(0, 2, 0);
        final Block base = CrystalUtil.mc.world.getBlockState(blockPos).getBlock();
        final Block b1 = CrystalUtil.mc.world.getBlockState(boost).getBlock();
        final Block b2 = CrystalUtil.mc.world.getBlockState(boost2).getBlock();
        if (base != Blocks.BEDROCK && base != Blocks.OBSIDIAN) {
            return false;
        }
        if (b1 != Blocks.AIR && !isReplaceable(b1)) {
            return false;
        }
        if (!newPlace && b2 != Blocks.AIR) {
            return false;
        }
        final AxisAlignedBB box = new AxisAlignedBB((double)blockPos.getX(), blockPos.getY() + 1.0, (double)blockPos.getZ(), blockPos.getX() + 1.0, blockPos.getY() + 3.0, blockPos.getZ() + 1.0);
        final List<Entity> entities = new ArrayList<Entity>(CrystalUtil.mc.world.loadedEntityList);
