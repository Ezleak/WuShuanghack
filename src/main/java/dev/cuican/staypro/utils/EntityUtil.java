...
public class EntityUtil {
    ...
    public static EnumFacing GetFacing() {
        switch (MathHelper.floor(EntityUtil.mc.player.rotationYaw * 8.0f / 360.0f + 0.5) & 0x7) {
            case 0:
            case 1: {
    ...
    public static EntityPlayer getTarget(final Integer range) {
        return (EntityPlayer)EntityUtil.mc.world.playerEntities.stream().filter(entityPlayer -> entityPlayer != EntityUtil.mc.player && !FriendManager.isFriend(entityPlayer) && !((EntityPlayer)entityPlayer).isDead && EntityUtil.mc.player.getDistance(entityPlayer) < range).min(Comparator.comparing(e -> EntityUtil.mc.player.getDistance(e))).get();
    }
    public static boolean isPlayerBurrow(final EntityPlayer player) {
        return !EntityUtil.mc.world.getBlockState(getPlayerPosFloored(player)).getBlock().equals(Blocks.AIR);
    }
