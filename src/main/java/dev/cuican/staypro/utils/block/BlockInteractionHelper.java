...
public class BlockInteractionHelper {
    ...
    public static boolean isAirBlock(final BlockPos pos) {
        return BlockInteractionHelper.mc.world.getBlockState(pos).getBlock().equals(Blocks.AIR);
    }

    public static boolean canBreak(final BlockPos pos) {
        return canBreak(BlockInteractionHelper.mc.world.getBlockState(pos), pos);
    }
    private static boolean canBreak(final IBlockState state, final BlockPos pos) {
        return state.getBlockHardness((World)BlockInteractionHelper.mc.world, pos) != -1.0f && state.getBlock() != Blocks.AIR && !state.getMaterial().isLiquid();
    }
