...
public class RenderUtils3D extends Tessellator {
    ...
    public static void drawText(final BlockPos pos, final float height, final String text) {
        GlStateManager.pushMatrix();
        CrystalUtil.glBillboardDistanceScaled(pos.getX() + 0.5f, pos.getY() + height, pos.getZ() + 0.5f, (EntityPlayer)Minecraft.getMinecraft().player);
        GlStateManager.disableDepth();
        GlStateManager.translate(-(FontManager.fontRenderer.getStringWidth(text) / 2.0), 0.0, 0.0);
        FontManager.fontRenderer.drawStringWithShadow(text, 0.0f, 0.0f, -1);
        GlStateManager.popMatrix();
    }
}